...
export interface ToastOptions {
  ...
  /**
   * Arbitrary user data attached to the toast. Use this to carry context
   * (e.g. an entity id) into custom render slots or `onDismiss` callbacks.
   *
   * NOTE: keep values JSON-serializable (strings, numbers, booleans, plain
   * objects/arrays). Avoid functions, Symbols, class instances, or circular
   * references — `meta` is persisted alongside flash messages via
   * `JSON.stringify` and a non-serializable value would either be dropped
   * silently or throw a "Converting circular structure to JSON" error.
   */
